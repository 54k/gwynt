...
import io.gwynt.core.nio.NioDatagramChannelConfig;
...
public class RudpNioServerChannel extends AbstractNioChannel implements MulticastChannel, ServerChannel {
    ...
    public RudpNioServerChannel(AbstractNioChannel parent) throws IOException {
        super(parent, DatagramChannel.open());
    }

    @Override
    protected Unsafe newUnsafe() {
        return new RudpNioServerChannelUnsafe();
    }
    ...
    private class RudpNioServerChannelUnsafe extends AbstractNioUnsafe<DatagramChannel> {

        private Map<SocketAddress, RudpNioChannel> channels = new HashMap<>();
        ...
        private void acceptChannel(SocketAddress address, byte[] message) {
            RudpNioChannel channel;

            if (!channels.containsKey(address)) {
                channel = new RudpNioChannel(RudpNioServerChannel.this, javaChannel());
                channels.put(address, channel);
                eventLoop().next().register(channel);
            } else {
                channel = channels.get(address);
            }

            channel.pipeline().fireMessageReceived(message);
        }
